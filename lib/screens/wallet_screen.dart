...
class _WalletScreenState extends State<WalletScreen>
  ...
  // ── Key to trigger refresh on the transactions widget ─────────
  final _txKey = GlobalKey<RecentTransactionsState>();
  ...
  Future<void> _loadRupeeOptions() async {
    try {
      ...
      if (mounted) setState(() { _rupeeOptions = options; _isLoading = false; });
    } catch (_) {
      if (mounted) setState(() {
  ...
  // Wraps the OUTER CustomScrollView in a RefreshIndicator.
  // Delegates the actual data refresh to RecentTransactions via GlobalKey.
  Future<void> _onRefresh() async {
    HapticFeedback.mediumImpact();
    await _txKey.currentState?.refresh();
  }
    ...
    return Scaffold(
      ...
      body: SizedBox.expand(
        child: Stack(children: [
          ...
          Positioned.fill(child: AnimatedBuilder(
            ...
            builder: (_, __) => Stack(children: [
              Positioned(
                ...
                child: _OrbGlow(
                  color: _gold.withOpacity(0.15 + _pulseAnim.value * 0.05),
                  size: 400,
                ),
              ),
              Positioned(
          ...
          Positioned.fill(child: AnimatedBuilder(
            ...
            builder: (_, __) => CustomPaint(painter: _ParticlePainter(
          ...
          // ── RefreshIndicator wraps the OUTER scroll view ───────
          RefreshIndicator(
            color: _gold,
            backgroundColor: _cardBg,
            strokeWidth: 2.5,
            displacement: 60,
            onRefresh: _onRefresh,          // ← triggers _txKey.refresh()
            child: CustomScrollView(
              ...
              slivers: [
                ...
                SliverToBoxAdapter(child: _AnimatedReveal(
                  animation: _entryAnim, delay: 0.05,
                  child: _buildQuickActions(),
                )),
                const SliverToBoxAdapter(child: SizedBox(height: 22)),
                SliverToBoxAdapter(child: _AnimatedReveal(
                  animation: _entryAnim, delay: 0.12,
                  child: const Padding(
                ...
                SliverToBoxAdapter(child: _AnimatedReveal(
                  animation: _entryAnim, delay: 0.20,
                ...
                SliverToBoxAdapter(child: _AnimatedReveal(
                  ...
                  child: Padding(
                    ...
                    // Pass the GlobalKey so we can call .refresh() on it
                    child: RecentTransactions(key: _txKey),
                  ),
                )),
      ...
      floatingActionButton: AnimatedBuilder(
        ...
        builder: (_, child) => Transform.translate(
          ...
          child: Container(
            decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(
                color: _gold.withOpacity(0.35 + _fabAnim.value * 0.15),
        ...
        child: GestureDetector(
          onTap: () { HapticFeedback.mediumImpact(); _showFundsSheet(context); },
          child: Container(
            height: 56,
            ...
            child: const Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.add_rounded, color: Color(0xFF1A1200), size: 22),
              SizedBox(width: 8),
              Text('Add Funds', style: TextStyle(
                  color: Color(0xFF1A1200), fontSize: 15,
        ...
        child: Row(children: [
          _AnimatedReveal(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('Manage your balance', style: TextStyle(
                  color: _textMuted, fontSize: 12, fontWeight: FontWeight.w500)),
            ]),
          ),
          const Spacer(),
          _AnimatedReveal(
            animation: _entryAnim, delay: 0.04,
            child: _GlassButton(
              onTap: () {},
    ...
    return Padding(
      ...
      child: Row(children: [
        Expanded(child: _QuickActionButton(
          ...
          onTap: () { HapticFeedback.selectionClick(); _showDepositSheet(context); },
        )),
        const SizedBox(width: 12),
        Expanded(child: _QuickActionButton(
          icon: Icons.arrow_upward_rounded, label: 'Withdraw',
          iconColor: _red,
          borderColor: _red.withOpacity(0.3),
          bgColor: _red.withOpacity(0.08),
          onTap: () { HapticFeedback.selectionClick(); _showComingSoon('Withdrawal'); },
        )),
        const SizedBox(width: 12),
        Expanded(child: _QuickActionButton(
  ...
  void _showFundsSheet(BuildContext context) {
    _showDarkSheet(context: context, child: Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        _SheetHandle(),
        const SizedBox(height: 20),
        const _SheetTitle(title: 'Manage Funds', subtitle: 'Choose an action below'),
        const SizedBox(height: 24),
        _SheetActionTile(
          icon: Icons.add_circle_rounded, iconColor: _green,
          iconBg: _green.withOpacity(0.1),
          title: 'Deposit', subtitle: 'Add money to your wallet',
          onTap: () { HapticFeedback.selectionClick(); _showDepositSheet(context); },
        ),
        const SizedBox(height: 10),
        _SheetActionTile(
          icon: Icons.remove_circle_rounded, iconColor: _red,
          iconBg: _red.withOpacity(0.1),
          title: 'Withdraw', subtitle: 'Transfer to your bank account',
          onTap: () { Navigator.pop(context); _showComingSoon('Withdrawal'); },
        ),
  ...
  void _showDepositSheet(BuildContext context) {
    ...
    _showDarkSheet(context: context, child: Column(
      ...
      children: [
        _SheetHandle(),
        const SizedBox(height: 20),
        const _SheetTitle(title: 'Add Money', subtitle: 'Select an amount to deposit'),
        const SizedBox(height: 24),
        GridView.count(
          crossAxisCount: 3, shrinkWrap: true,
          physics: const NeverScrollableScrollPhysics(),
          crossAxisSpacing: 12, mainAxisSpacing: 12, childAspectRatio: 1.6,
          children: _rupeeOptions.map((amount) => _AmountChip(
            amount: amount,
            onTap: () { HapticFeedback.selectionClick(); _showDepositSheet(context); },
          )).toList(),
        ),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              const Text('Enter custom amount', style: TextStyle(
  ...
  void _showDarkSheet({required BuildContext context, required Widget child}) {
...
class _QuickActionButton extends StatefulWidget {
  ...
  final Color iconColor, borderColor, bgColor;
  final VoidCallback onTap;

  const _QuickActionButton({Key? key,
    required this.icon, required this.label,
    required this.iconColor, required this.borderColor,
    required this.bgColor, required this.onTap,
  }) : super(key: key);

  @override State<_QuickActionButton> createState() => _QuickActionButtonState();
}
    ...
    return GestureDetector(
      ...
      child: AnimatedBuilder(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: widget.bgColor, borderRadius: BorderRadius.circular(18),
          ...
          child: Column(children: [
            ...
            Text(widget.label, style: TextStyle(
                color: widget.iconColor, fontSize: 12, fontWeight: FontWeight.w700)),
          ]),
        ),
      ),
    );
...
class _AmountChip extends StatefulWidget {
  final int amount; final VoidCallback onTap;
  const _AmountChip({Key? key, required this.amount, required this.onTap})
      : super(key: key);

  @override State<_AmountChip> createState() => _AmountChipState();
}
...
class _AmountChipState extends State<_AmountChip>
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTapDown: (_) { _ctrl.forward(); setState(() => _selected = true); },
      onTapUp: (_) {
        _ctrl.reverse();
      ...
      onTapCancel: () { _ctrl.reverse(); setState(() => _selected = false); },
      child: AnimatedBuilder(
        animation: _ctrl,
        builder: (_, child) =>
            Transform.scale(scale: 1.0 - _ctrl.value * 0.04, child: child),
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            gradient: _selected ? const LinearGradient(
          ...
          child: Center(child: Text('₹${widget.amount}', style: TextStyle(
            color: _selected ? const Color(0xFF1A1200) : _textPrimary,
...
class _SheetTitle extends StatelessWidget {
  ...
  const _SheetTitle({Key? key, required this.title, required this.subtitle})
      : super(key: key);
...
class _SheetActionTile extends StatefulWidget {
  ...
  const _SheetActionTile({Key? key,
    required this.icon, required this.iconColor, required this.iconBg,
    required this.title, required this.subtitle, required this.onTap,
  }) : super(key: key);

  @override State<_SheetActionTile> createState() => _SheetActionTileState();
}
        ...
        child: Container(
          ...
          child: Row(children: [
            Container(
              width: 46, height: 46,
              decoration: BoxDecoration(
                  color: widget.iconBg, borderRadius: BorderRadius.circular(14)),
...
class _AnimatedReveal extends StatelessWidget {
  ...
  const _AnimatedReveal({Key? key,
    required this.animation, required this.delay, required this.child,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      animation: animation,
      builder: (_, __) {
        final denom  = (1.0 - delay).clamp(0.001, 1.0);
        final t      = ((animation.value - delay) / denom).clamp(0.0, 1.0);
...
class _NoisePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 1600; i++) {
      paint.color = Colors.white.withOpacity(rng.nextDouble() * 0.5);
      canvas.drawCircle(
        Offset(rng.nextDouble() * size.width, rng.nextDouble() * size.height),
        rng.nextDouble() * 0.65, paint,
      );
    }
  }

  @override bool shouldRepaint(_NoisePainter old) => old.seed != seed;
}